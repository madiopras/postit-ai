...
/**
 * Seed sample FAQ and SOP content for Coffe Gaul.
 *
...
const FAQ_SEED: Array<{ question: string; answer: string; category: string }> =
  [
    ...
    {
      question:
        "Apakah Coffe Gaul menyediakan kopi untuk subscription bulanan?",
      answer:
...
async function main() {
  ...
  console.log(`🌱 Seeding ${FAQ_SEED.length} FAQ Coffe Gaul...`);
  ...
  console.log(`🌱 Seeding ${SOP_SEED.length} SOP Coffe Gaul...`);
  const seedOwner = await db.query.users.findFirst({
    where: eq(users.role, "super_admin"),
  });
  if (!seedOwner) {
    throw new Error(
      "A super_admin account is required before seeding SOP versions",
    );
  }
  ...
  for (const entry of SOP_SEED) {
    ...
    if (status === "error")
      throw new Error(`Failed to publish seeded SOP: ${sop.title}`);
    console.log(`   ✓ ${sop.title}`);
  }

  console.log(
    "✅ Seed done — 50 FAQ & 10 SOP Coffe Gaul embedded and published.",
  );
}
...
main()
  ...
  .catch((error) => {
    ...
    console.error(
      "   An embedding endpoint must be reachable — check ROUTER_BASE_URL in .env",
    );
    process.exit(1);
  });
